...
isolated function testBatchArchiveAssociations() returns error? {
    ...
    test:assertTrue(response.status == "COMPLETE", "Expected response status to be COMPLETE");
}
...
isolated function testHighUsageReport() returns error? {
    ...
    test:assertTrue(reportIdValue.length() > 0, "Expected reportId to be present");
}
