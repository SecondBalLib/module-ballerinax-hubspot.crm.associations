...
isolated function testHighUsageReport() returns error? {
    ...
    anydata reportIdAnydata = response["reportId"];
    string? reportIdValue = reportIdAnydata is string ? reportIdAnydata : ();
    test:assertTrue(reportIdValue is string && reportIdValue != "", "Expected reportId to be present");
}
